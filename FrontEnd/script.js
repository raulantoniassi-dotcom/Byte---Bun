...
async function carregarCardapio() {
  try {
    ...
    popularSelect(
      "selectPao",
      dados.filter((item) => item.categoria === "pao"),
    );
    popularSelect(
      "selectRecheio",
      dados.filter((item) => item.categoria === "recheio"),
    );
    popularSelect(
      "selectMolho",
      dados.filter((item) => item.categoria === "molho"),
    );
  } catch (erro) {
    cupom.textContent =
      "Não foi possível carregar o cardápio. O servidor está rodando?";
  }
}
...
document
  .querySelector("#btnCalcularPedido")
  .addEventListener("click", async function () {
    ...
    try {
      ...
      cupom.textContent =
        "Não foi possível enviar o pedido. O servidor está rodando?";
    }
  });
